...
if run_btn:
    ...
    NODE_LABELS = {
        "router":      ("🔀", "Routing — deciding if research is needed…"),
        "research":    ("🔎", "Researching the web for up-to-date information…"),
        "orchestrator":("🧩", "Planning blog structure and sections…"),
        "worker":      ("✍️",  "Writing sections in parallel…"),
        "reducer":     ("🖼️",  "Merging content and generating images…"),
    }
    ...
    for kind, payload in try_stream(app, inputs):
        if kind in ("updates", "values"):
            ...
            if node_name and node_name != last_node:
                icon, label = NODE_LABELS.get(node_name, ("⚙️", f"Running {node_name}…"))
                status.update(label=f"{icon} {label}", expanded=True)
                last_node = node_name

            # Build clean progress card
            plan_obj = current_state.get("plan")
            if isinstance(plan_obj, dict):
                sections_total = len(plan_obj.get("tasks", []))
            sections_done = len(current_state.get("sections", []) or [])
            evidence_count = len(current_state.get("evidence", []) or [])
            mode = current_state.get("mode") or "—"
            needs_research = current_state.get("needs_research", False)

            lines = []
            lines.append(f"**Mode:** `{mode}`  |  **Research:** {'Yes' if needs_research else 'No'}")
            if evidence_count:
                lines.append(f"**Sources found:** {evidence_count}")
            if sections_total:
                lines.append(f"**Sections:** {sections_done} / {sections_total} written")
            progress_area.info("\n\n".join(lines))
